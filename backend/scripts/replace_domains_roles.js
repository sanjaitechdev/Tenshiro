...
async function replaceDomainRoleData() {
    ...
    try {
        ...
        const seedPath = path.join(__dirname, '../database/seed_domains_roles.sql');
        const seedSql = fs.readFileSync(seedPath, 'utf8');

        // Execute the seed file
        console.log('🗑️  Clearing old data and inserting new domains and roles...');
        ...
        console.log('\n✨ Database update complete! Your PathPilot AI now has fresh domain and role data.');

    } catch (error) {
        console.error('❌ Error replacing domain/role data:', error);
        process.exit(1);
    } finally {
